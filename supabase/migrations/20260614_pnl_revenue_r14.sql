-- get_pnl_by_period v3: revenue = R14 (по цене карточки), сверено с эталоном владелицы
--
-- Эталон Excel за 01-07.12.2025: R14 «Продано по цене карточки» = 170 768
-- БД (retail_price × qty − returns) = 169 925 (Δ -0.5% ✅)
-- Маржа на этих данных = 15.7% (реалистично для WB-селлера)
--
-- До этого revenue считался как retail_amount (после СПП-скидки WB) = 113 948,
-- маржа выходила 38-50%, что нереально.
