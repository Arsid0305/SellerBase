-- Ставка налога в периодном P&L — из app_settings, а не из тела функции.
--
-- Было `retail_amount * 0.06` прямо в SQL, тогда как дневной расчёт читал
-- app_setting_num('tax_rate'). Значение совпадает (0.06), поэтому цифры
-- не меняются — проверено на золотой неделе: выручка 170 767,86 ₽,
-- прибыль 74 067,23 ₽, расхождение с дневным 0,00.
--
-- Смысл правки: ставка перестаёт жить в двух местах по-разному. Изменится
-- режим налогообложения — правится одна строка в app_settings, а не тело
-- функции, о котором никто не вспомнит.
--
-- Тело функции ниже — прежнее, изменена единственная строка расчёта tax_rub.
CREATE OR REPLACE FUNCTION public.get_pnl_by_period(p_from date, p_to date)
 RETURNS TABLE(sku_id bigint, my_article text, wb_article bigint, revenue_rub numeric,
               commission_rub numeric, logistics_rub numeric, penalty_rub numeric,
               units_sold numeric, cogs_rub numeric, tax_rub numeric,
...
AS $function$
  WITH cogs AS (
    ...
    WHERE r.rr_dt::date BETWEEN p_from AND p_to AND r.doc_type_name IN ('Продажа','Возврат')
    GROUP BY s.id
  ),
  rev AS (
    SELECT nm_id,
  ...
  exp AS (
    ...
    FROM public.wb_reports_fact
    WHERE rr_dt::date BETWEEN p_from AND p_to GROUP BY nm_id
  ),
  ...
  joined AS (
    SELECT s.id AS sku_id, s.my_article, COALESCE(s.wb_article, n.nm_id) AS wb_article,
      ...
      COALESCE(rev.retail_amount_rub, 0) * public.app_setting_num('tax_rate') AS tax_rub
    FROM nm_all n
