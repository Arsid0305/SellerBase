-- get_pnl_by_period: учесть пропущенные статьи расходов
-- storage_fee, acquiring_fee, deduction, rebill_logistic_cost
-- До фикса маржа на /pnl и /dashboard была завышена (~50% при реальных 15-20%)
...
AS $function$
  ...
  joined AS (
    SELECT s.id AS sku_id, s.my_article, s.wb_article,
      ...
      COALESCE(exp.commission_rub, 0) AS commission_rub,
    ...
    WHERE COALESCE(rev.revenue_rub, 0) > 0
       OR COALESCE(exp.commission_rub, 0) > 0
       OR COALESCE(exp.logistics_rub, 0) > 0
       OR COALESCE(exp.penalty_rub, 0) > 0
  ...
  SELECT
    ...
    cogs_rub,
    tax_rub,
    revenue_rub - commission_rub - logistics_rub - penalty_rub - storage_rub
      - acquiring_rub - deduction_rub - cogs_rub - tax_rub AS net_profit_rub,
    CASE WHEN revenue_rub > 0 THEN
      (revenue_rub - commission_rub - logistics_rub - penalty_rub - storage_rub
        - acquiring_rub - deduction_rub - cogs_rub - tax_rub) / revenue_rub * 100
