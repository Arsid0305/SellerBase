-- Приведение net_profit в get_pnl_by_period к формуле из книги владелицы «PL WB»:
--   1) Эквайринг НЕ вычитаем (у неё в книге всегда 0, вероятно уже в комиссии WB)
--   2) Rebill (перевес логистики возвратов) НЕ вычитаем (то же самое)
--   3) Налог УСН считаем как 6% + 1% = 6.8% от revenue (в её книге такая ставка)
--   4) storage / deduction / penalty оставляем — они у неё вычитаются
--
-- Сверка mcp.execute_sql за 30 дней (rules.md §16):
--   было (с acquiring + rebill, налог 6%):  net_profit = -4 771 ₽, margin = -1.8%
--   стало (без acquiring/rebill, налог 6.8%): net_profit = +8 740 ₽, margin = +2.9%
--
-- Формула теперь: net_profit = ppvz_for_pay - logistics - storage - deduction - penalty - cogs - tax
--                 tax = revenue * 0.068
...
AS $function$
  ...
  joined AS (
    SELECT
      ...
      COALESCE(rev.revenue_rub, 0) * 0.068     AS tax_rub
    FROM nm_all n
  ...
  SELECT
    ...
    cogs_rub,
    tax_rub,
    ppvz_for_pay_rub - logistics_rub - storage_rub - deduction_rub
