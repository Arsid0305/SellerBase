-- Фикс #11 (маржа завышена в 3-5×): exp CTE в get_pnl_by_period фильтровал
-- doc_type_name IN ('Продажа','Возврат'), но logistics/storage/deduction/rebill
-- в wb_reports_fact приходят отдельными строками с ПУСТЫМ doc_type_name.
-- Из-за фильтра все эти расходы выпадали → margin завышалась с реальных 30%+ до 56%.
--
-- Сверка через mcp.execute_sql до/после (rules.md §16):
--   до:    revenue 194 488 | logistics 0 | net_profit 109 500 | margin 56.3%
--   после: revenue 194 488 | logistics 47 021 | net_profit 58 843 | margin 30.3%
...
AS $function$
  ...
  exp AS (
    -- ВСЕ расходы за период (без фильтра по doc_type_name):
    -- продажи/возвраты несут acquiring, а строки с пустым doc_type_name несут logistics/storage/deduction/rebill.
    SELECT nm_id,
