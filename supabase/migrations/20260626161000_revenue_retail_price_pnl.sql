-- Доход = моя цена в карточке × количество, без СПП ВБ.
-- СПП компенсирует ВБ сам (ppvz_for_pay > retail_amount подтверждает),
-- поэтому revenue считаем по retail_price * quantity, а не по retail_amount.
-- Все 3 RPC (pnl + catalog_sales_daily + sales_report_daily) переводим единой формулой.
