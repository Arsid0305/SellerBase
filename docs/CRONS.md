...
| `fetch-wb-funnel-aggregate-daily` | `0 4 * * *`     | 07:00   | `fetch-wb-funnel-aggregate`| Пересчитывает воронку (заказы → выкуп) за последние 30 дней (совпадает с окном WB-кабинета). | `20260611_wb_sales_funnel_period.sql`          |
