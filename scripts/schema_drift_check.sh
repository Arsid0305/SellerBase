...
# `|| true` не для красоты. Когда долг закрыт, в baseline остаются одни
# комментарии, grep не находит ни строки и возвращает 1 — а скрипт идёт
# под `set -e` и молча обрывается ровно здесь. 12.09.2026 так и вышло:
# долг закрыли, список опустел, и сверка упала без единого слова о причине,
# напечатав только счётчики. Пустой список — это успех, а не ошибка.
grep -v '^#' "$baseline" | sed '/^$/d' | sort -u > "$work/known.h" || true
touch "$work/known.h"
