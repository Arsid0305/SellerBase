...
def main():
    ...
    for line in [
        ...
        ["Что значит ячейка", "Значение — вносим его. «верно, не трогать» — поле проверено "
                              ...
                              "«нужен ответ владелицы» — вопрос к вам, он выписан в конце "
                              "соответствующего файла разбора. «нужна фотография цвета» — ждём снимок "
                              "товара. «сценария нет» — работа ещё не сделана. «карточки нет "
                              "в каталоге» — товар в кабинете отсутствует. Пустых ячеек в книге "
                              "нет: если поле не заполнено, ячейка говорит почему. "
                              "Обоснования — в docs/seo/descriptions/."],
        ...
        ["Игрушки, скакалки,\nфитнес-резинки", "Три новые группы, собраны 02.09 по трём источникам сразу: годовой "
                     "отчёт Джема, срез Эвирмы и кластеры Промопульта. Конструкции мячей и канатов "
                     "сняты с ваших фотографий слайдов; по скакалкам и резинкам фото не было — "
                     "конструкция взята из полей кабинета, перед заливкой описаний сверьте с товаром."],
        ["Мячи для собак", "Наименования меняются с «на верёвке» на «на резинке»: подвес эластичный, "
                           "и покупатели ставят единицы именно за это несоответствие. Слово "
                           "«суперпрочный» убрано из комплектации — мяч полый, и отзывы бьют ровно "
                           "в это слово. Обоснование — docs/seo/descriptions/igrushki-dlya-zhivotnyh.md."],
        ["Цены против рынка", "Эвирма 02.09: скакалки у нас 1 799 ₽ при медиане рынка 345 ₽ и максимуме "
                              "топ-100 в 1 426 ₽; фитнес-резинки ACRB3FR101CL — 2 659 ₽ при максимуме "
                              "топ-100 в 2 239 ₽. Это вопрос к цене до рекламы, а не к карточке."],
        ...
        ["Размеры и вес", "Отдельный лист по всему каталогу: габариты предмета и упаковки, вес "
                          "нетто и брутто рядом, с колонкой «Что не так». Брутто взят из кабинета "
                          "и сверен с вашей выгрузкой «Общие характеристики одним файлом». "
                          "У семи карточек он МЕНЬШЕ нетто — так быть не может, смотрите эти строки."],
        ...
        ["Листы без разбора", "Четыре листа — «Бандажи косметические», «Ножи для пиццы», "
    ...
    # Ключ «_отложено» перечисляет группы, которые владелица просила не трогать.
    # Данные по ним в снимке остаются — лист вернётся одной правкой этого ключа,
    # заново выгружать кабинет не придётся.
    postponed = SNAPSHOT.get("_отложено", {})
    for name in SNAPSHOT:
        if name == "_отложено" or name in postponed:
            continue
