...
## Работа с GitHub — правило от 09.09.2026

Свод меняется в AI_OS и потом раскатится на все репо. Здесь — то, что уже действует.

**Один PR на сессию.** Не заводим PR под каждую задачу: копим работу в одной ветке
`claude/...` и собираем один PR. Если PR сессии уже смержен, а работа продолжилась —
ветка начинается заново от `main`, и это новый PR сессии.

**Автомержа нет.** `automerge.yml` удалён 09.09.2026. Раньше он включал нативный
auto-merge на каждый PR из `claude/*`, и PR уезжал в `main` сам.

**Мержит владелица, руками.** Моя работа заканчивается так: ветка готова, проверки
пройдены, PR открыт — и я даю ссылку. Дальше кнопку нажимает она.

Если авто-мерж где-то всплывёт снова (включён в настройках репозитория или вернулся
workflow) — снять его с PR и сказать владелице, а не мержить.

---

## Как говорить с владелицей

Просьба от 09.09.2026: **меньше технического языка**. Владелица не программист.
...
- Короче. Одна мысль — одно предложение.

Пример. Вместо разбора про исчерпание пула соединений и 504 — «Сводка каждый день
теряла две строки. Бот дёргал базу слишком резко, теперь делает это по очереди.
Проверил: обе строки зелёные».
...
| GitHub Actions| ✅ работают (ограничения сняты 09.09.2026) |
| .env реальный | ❌ (секреты в GitHub Secrets / Supabase Vault) |

### Деплой и запуск Edge Functions

**Деплой — через MCP `mcp__Supabase__deploy_edge_function`.** Деплой Edge Functions
идёт мимо CI в любом случае — из сессии, а не из workflow. Передавать нужно и общие
модули: для функции,
...
**Одна точка входа на контур** — `tasks/rules.md` §26. Новый файл в разделе называется
в его точке входа тем же коммитом; второго входа в ту же тему не заводим.
