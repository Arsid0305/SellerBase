...
export default async function SuppliesPage() {
  const plans = await fetchSupplyPlans();
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <PageHeader
          title="Поставки"
          description="План отгрузок в фулфилмент и закупок на 1688"
        />
        <Button asChild>
      ...
      {plans.length === 0 ? (
        <div className="rounded-xl border border-dashed bg-card/40 px-6 py-16 text-center">
          <p className="text-sm text-muted-foreground">Поставок ещё нет.</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Создайте первую — система предложит рекомендации по каждому складу WB.
          </p>
        </div>
      ) : (
