...
const FIELDS_TOOLTIP =
  'Сколько характеристик предмета заполнено. Список полей берётся с WB (content/v2/object/charcs), поэтому «из скольки» — это то, что площадка действительно ждёт в этой категории, а не наша выдумка. Поля для продавцов других стран (ИКПУ, NTIN) исключены. «Не видим» — поля, которые мы с WB пока не тянем: ТН ВЭД, сертификаты, НДС. Про них ответ «неизвестно», а не «пусто», и в процент они не входят.';
