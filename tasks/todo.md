...
- [ ] **Telegram алерты не приходят** — функция вернула `telegram_sent: false`. Возможно секреты `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` не видны Edge Function (пробел в имени или сохранены под другими именами). Задеплоен debug-endpoint `?debug=env` — после следующего мержа вызвать `https://hcebwgjgppwaguqittpi.supabase.co/functions/v1/telegram-alerts?debug=env`, посмотреть какие имена реально есть в env и подставить нужные в код.
