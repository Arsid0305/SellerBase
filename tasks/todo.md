...
- 🟡 **Связь воронки с карточкой — сделана наполовину 22.08.**
  ...
  - 🔴 **По дороге найден дефект сбора.** `fetch-wb-funnel` читал поле `addToCartCount`,
    которого у WB нет — реальное имя `cartCount`. Корзина лежала нулём **с начала сбора,
    с 4 июня**. Исправлено, задеплоено (v31), заодно добавлены `addToWishlistCount`,
    `addToCartConversion`, `cartToOrderConversion`, `buyoutPercent`. Перезабор идёт
    окнами: WB отвечает `400 excess limit on days` на диапазон в 30 дней.
