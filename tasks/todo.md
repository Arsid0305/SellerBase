...
- [ ] **WB-style график СВЕРХУ** над PnLChart (компонент готов, подключить на /dashboard)
- [ ] **Tooltip на PnLChart** — реализован, проверить визуально после деплоя
- [ ] **«vs позавчера» в брифе** — выбрать формулировку (конкретная дата / «к предыдущему дню» / «vs прошлый понедельник»)
- [ ] **Фото в Топ-5 / Категории** — проверить что подгружаются через `tm/1.webp`
- [ ] (накидывает пользователь)

### 🔴 Открытое из критики

- [ ] **Stocks API миграция** — дедлайн **23.06.2026**, сейчас 401 каждый день
