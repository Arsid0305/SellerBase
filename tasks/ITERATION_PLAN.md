...
- [ ] **WB-style график СВЕРХУ на /dashboard** (большая фича):
  - PnLChart остаётся ниже (нравится, не трогаем).
  - Новый блок ВЫШЕ PnLChart — точная копия WB-кабинета.
  ...
  - [ ] Подключить на /dashboard над `PnLChart`
  - [ ] Заглушки для «Заказы» и «Продвижение» (нет данных)
  - [ ] Для «Приход и расход» — наша формула с cogs+tax (WB не считает себес)
